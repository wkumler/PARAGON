...
### Fix problematic areas from IS contamination and write out
...
```{r write out}
decontam_areas %>%
  filter(!(compound_name=="O-Acetylcarnitine" & str_detect(iso_name, "13C8"))) %>%
  filter(!(compound_name=="Trigonelline" & str_detect(iso_name, "13C6"))) %>%
  write_csv("targeted/all_peak_areas_w_isos.csv")
```
...
fixed_IS <- parametadata %>%
  filter(samp_type%in%c("Smp", "Poo", "Blk", "Bad")) %>%
  filter(IS_type=="wIS") %>%
...
env_concs <- wIS_areas %>%
  ...
  inner_join(parametadata %>% filter(samp_type%in%c("Smp", "Bad")) %>% distinct(filename)) %>%
  left_join(chosen_IS) %>%
  left_join(IS_areas) %>%
  ...
  select(compound_name, filename, conc_nm_in_env)
...
## Putting it all together nicely and saving into a clean state

```{r}
iso_areas <- read_csv("targeted/all_peak_areas_w_isos.csv") %>%
  left_join(parametadata %>% distinct(filename, samp_type, amendment)) %>%
  filter(samp_type%in%c("Smp", "Bad")) %>%
  select(-samp_type) %>%
  # filter(compound_name%in%c("Glycine betaine", "Guanine", "L-Glutamine")) %>%
  group_by(compound_name, amendment) %>%
  complete(filename, iso_name, fill = list(area=0)) %>%
  group_by(filename, compound_name) %>%
  mutate(rel_area=area/sum(area)) %>%
  mutate(rel_area=rel_area/rel_area[str_detect(iso_name, "13C0, 15N0")]) %>%
  ungroup() %>%
  select(filename, compound_name, iso_name, area, rel_area) %>%
...
bad_cmpds <- c("Melamine", "Uridine", "Itaconic acid?", "Inosine", "Dexpanthenol", 
               "Choline sulfate", "5-Hydroxyectoine", "L-Tyrosine", "Urea", "Homarine",
               "Acetylcholine", "L-Valine", "Oxalic acid", "Malic acid")
maa_names <- c("Also Palythene/Usujirene?", "Shinorine?", "Mycosporine-glycine?", 
               ...
               "Palythenic acid?")
clean_complete <- left_join(iso_areas, env_concs) %>%
  mutate(conc_nm_in_env=conc_nm_in_env*rel_area) %>%
  select(shortname, compound_name, iso_name, area, conc_nm_in_env) %>%
  filter(!compound_name%in%bad_cmpds) %>%
  left_join(parametadata %>% distinct(shortname, depth)) %>%
  filter(!(compound_name%in%maa_names & depth=="Deep")) %>%
  select(-depth)
write_csv(clean_complete, "targeted/clean_complete.csv")
```
