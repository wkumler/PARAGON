...
Samples were taken during the 2021 SCOPE PARAGON cruise near Station ALOHA that targeted a chlorophyll bloom within an anticyclonic eddy. The bloom was initially dominated by the diatom genus *Hemiaulus* with a large surface chlorophyll signal that decayed into a more typical North Pacific Subtropical Gyre community dominated by *Prochlorococcus* and *SAR11* over the course of the two-week time series Supplemental figure XX). Samples collected from 25 meters were well within the mixed layer and had PAR levels of 20-35% of the surface light while the 175 meter samples had a PAR level between 0.05 and 0.1%. Nitrate, nitrite, and ammonia data indicated that inorganic N was low in the surface (<0.01 µM), while the deeper samples at 175 meters were collected in the middle of the nutricline, with a clear deep chlorophyll maximum at 125 meters, an ammonia maximum at 150 meters, and a nitrite max at 160-165 meters. Environmental nitrate concentrations were about 0.5 µM at 175 meters.
...
bind_rows(chl_data, cnp_data, fcm_data, nut_data) %>%
  left_join(sum_data) %>%
  filter(name!="TDP (uM)") %>%
  mutate(name=factor(name, levels=c(
    ...
    "Nitrate (uM)", "Nitrite (uM)", "Ammonia (uM)", "Total diss. nitrogen (uM)",
    "Diss. organic N (uM)", "Diss. organic P (uM)", "Phosphate (uM)", "Silicate (uM)",
    "Prochlorococcus\n(cells / uL)", "Synechococcus\n(cells / mL)", "Picoeukaryotes\n(cells / mL)", "Het. bacteria\n(cells / uL)"
  ))) %>%
  ggplot(aes(x=value, y=depth, group=cast, color=datetime)) +
  geom_hline(yintercept = c(25, 175)) +
  geom_vline(xintercept = 0) +
  geom_path() +
  geom_point() +
  ...
  scale_y_reverse() +
  scale_x_continuous(expand = expansion(mult = c(0.05, 0.1)), n.breaks = 4) +
...
ggsave("manuscript/supplement/cruise_metadata.pdf", width = 6.5, height = 9)
```
