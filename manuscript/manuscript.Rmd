...
top_cmpds <- samp_concs %>%
  ...
  slice(1:14) %>%
  pull(compound_name)
total_conc_gp <- samp_concs %>%
  left_join(parametadata %>% distinct(shortname, samp_type, depth)) %>%
  filter(samp_type=="Smp") %>%
  filter(depth=="Surf") %>%
  mutate(compound_name=str_remove_all(compound_name, "^L-")) %>%
  mutate(compound_name=str_remove(compound_name, "/beta-Glutamic acid")) %>%
  ...
  facet_grid(~amendment, scales="free_x") +
  ...
  labs(x=NULL, y="Average metabolite surface\nconcentration (nM)", fill="Compound name", alpha="Isotope state") +
  theme_bw() +
  guides(alpha=guide_legend(override.aes = list(note="custom"))) +
  theme(axis.ticks.x=element_blank(), axis.text.x = element_blank())
total_conc_gp

xamend_metab_label_gp <- labeled_fracs %>%
...
xamend_metab_label_gp

xamend_metab_gp <- total_conc_gp / xamend_metab_label_gp + plot_layout(heights = c(0.4, 0.6))
