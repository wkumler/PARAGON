...
labeled_fracs %>%
  ...
  mutate(startime=factor(startime, levels=c("Morn", "Eve"), labels=c("6AM start", "6PM start"))) %>%
  ggplot(aes(x=timepoint, y=labeled_frac, color=startime)) +
  geom_vline(xintercept = 1:5, color="grey80") +
  geom_boxplot(fill="grey95") +
  geom_point(aes(fill=startime), position = position_dodge(width=0.7), color="black", pch=21) +
  scale_color_manual(values=c("#e0670b", "#580770"), breaks=c("6AM start", "6PM start"),
                     aesthetics = c("color", "fill")) +
  ggpubr::stat_compare_means(label.y = 0.9, method = "t.test", 
                             label = "p.signif", hide.ns = TRUE) +
  facet_grid(compound_name~amendment) +
  scale_y_continuous(limits=c(0, 1), labels=scales::label_percent()) +
  labs(y="Fraction labeled with at least one 15N atom", x=NULL, color=NULL, fill=NULL) +
  theme_bw()
...
labeled_fracs %>%
  ...
  filter(samp_type=="Smp") %>%
  filter(amendment=="Amm") %>%
  ggplot(aes(x=timepoint_num, y=labeled_frac, color=startime, fill=startime)) +
  geom_smooth(method="nlsLM", formula=y~a*x^b, method.args=list(start = c(a=1, b=1)), se=FALSE) +
  # geom_smooth(method="nlsLM", formula=y~(a*x)/(b+x), method.args=list(start = c(a=1, b=1)), se=FALSE) +
  # geom_smooth(method="nlsLM", formula=y~(a*x)/(b+x), method.args=list(start = c(a=1, b=1)), se=FALSE) +
  # geom_smooth(method="glm", formula=y~x, method.args=list(family="quasibinomial")) +
  geom_point(color="black", pch=21, size=3, alpha=0.5, stroke=1) +
  facet_wrap(~compound_name, ncol=2) +
  scale_y_continuous(limits=c(0, 1), labels=scales::label_percent()) +
  scale_x_continuous(breaks = c(0, 3, 10, 26)) +
  scale_color_manual(values=c("#e0670b", "#580770"), breaks=c("Morn", "Eve"), aesthetics = c("fill", "color")) +
  labs(x="Hours after 15NH4 addition", y="Fraction labeled with at least one 15N atom") +
  theme_bw()
...
peak_areas %>%
  ...
  mutate(n_N=paste0(str_extract(iso_name, "(?<=15N)\\d"), "x15N")) %>%
  mutate(area=area/1e4) %>%
  ggplot() +
  geom_col(aes(x=tripl, y=area, fill=n_N), color="black", width=1) +
  facet_nested(compound_name~amendment+startime+timepoint, scales="free_y") +
  scale_x_discrete(expand = expansion()) +
  scale_y_continuous(expand = expansion(mult=c(0, 0.1))) +
  labs(x=NULL, y="Peak area (x10^4)", fill="Number of\n15N labels") +
  theme_bw() +
