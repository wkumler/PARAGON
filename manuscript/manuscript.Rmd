...
library(ggtext)
library(minpack.lm)
options(pillar.sigfig = 7, pillar.print_max = 100)
...
Make plots from Rhea's data? Would love to add her as collaborator

Maybe just ref to PARAGON synthesis paper, should include Angel's IFCB data showing mostly Hemiaulus and its disappearance over the course of the cruise

Ref to Fuyan's published paper or remake the graph here?
...
cbind(
  peak_areas %>%
    # filter(iso_name=="L-Glutamine, 13C0, 15N0") %>%
    filter(iso_name=="Glycine betaine, 13C0, 15N0") %>%
    filter(str_detect(filename, "T0")) %>%
    select(filename, area),
  read_csv("targeted/wIS_areas.csv") %>%
    # filter(compound_name=="L-Glutamine") %>%
    filter(compound_name=="Glycine betaine") %>%
    filter(str_detect(filename, "T0")) %>%
    select(wIS_filename=filename, wIS_area=area)
) %>%
  # with(lm(wIS_area~area)) %>% summary()
  ggplot() +
  geom_point(aes(x=area, y=wIS_area)) +
  geom_abline(slope = 1.5, intercept = 0)
```


Total fraction of PN that was visible in metabolites, maybe big stacked barplot like usual?

Total number of compounds labeled over time (line plot?)
...
Organics seem to be largely used for ammonia rather than as-is
  ...
  - Creatine for being mostly 2x15N 1x13C labeled from arginine
...
Untargeted metabolites? Clustering their % labeled patterns with the targeted metabolites to see what groups they might belong to
