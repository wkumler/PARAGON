...
```{r supp fig cruise metadata plots}
...
metadata_gp <- bind_rows(chl_data, cnp_data, fcm_data, nut_data) %>%
  ...
  mutate(name=factor(name, levels=c(
    "ctd_chl", "PC_um", "PN_um", "C2N",
    "NO3 (uM)", "NO2 (uM)", "NH4 (uM)", "TDN (uM)",
    "DON (uM)", "DOP (uM)", "SRP (uM)", "Si (uM)",
    "pro_mL", "syn_mL", "peuk_mL", "het_mL"
  ), labels = c(
    "CTD fluorescence", "Particulate C (uM)", "Particulate N (uM)", "PC:PN ratio",
    "Nitrate (uM)", "Nitrite (uM)", "Ammonia (uM)", "Total dissolved\nnitrogen (uM)",
    "Dissolved\norganic N (uM)", "Dissolved\norganic P (uM)", "Phosphate (uM)", "Silicate (uM)",
...
sample_plan_gp <- ggplot() +
  ...
  annotate("text", x = as.POSIXct("2025-07-24 12:00:00"), y=4.6, label="Nitrate (3µM)",
           color=scales::hue_pal()(4)[1], vjust=0, hjust=0.2) +
  annotate("text", x = as.POSIXct("2025-07-27 12:00:00"), y=4.6, label="GMP (0.5µM)",
           color=scales::hue_pal()(4)[2], vjust=0, hjust=0.2) +
  annotate("text", x = as.POSIXct("2025-07-30 18:00:00"), y=4.6, label="Arginine (0.5µM)",
           color=scales::hue_pal()(4)[3], vjust=0, hjust=0.2) +
  annotate("text", x = as.POSIXct("2025-08-03 00:00:00"), y=4.6, label="Ammonia (3µM)",
           color=scales::hue_pal()(4)[4], vjust=0, hjust=0.2) +
...
```{r combine plan and cruise metadata}
combined_plot <- sample_plan_gp / metadata_gp + plot_layout(heights = c(0.2, 0.8))

ggsave("manuscript/supplement/cruise_metadata.png", combined_plot, width = 6.5, height = 8)
```
