...
all_areas <- map(c("pos", "neg"), function(pol_i){
  pol_i_areas <- map(c("Amm", "Nit", "GMP", "Arg"), function(amendment_i){
    file_path <- paste0("C:/Users/willi/Documents/PARAGON/untargeted/", amendment_i, "_", 
...
```{r}
msnexp_filled <- readRDS("untargeted/Amm_pos_noIS_output/msnexp_filled.rds")
summarized_feats <- msnexp_filled %>%
  makeXcmsObjFlat(revert_rts = FALSE) %>%
  summarise(feat_mzmed=unique(feat_mzmed), feat_rtmed=unique(feat_rtmed),
            feat_into=mean(into, na.rm=TRUE), mean_cor=mean(beta_cor, na.rm=TRUE),
            mean_snr=mean(beta_snr, na.rm=TRUE), n=n(), .by=feature)
summarized_feats %>%
  ggplot() +
  geom_point(aes(x=feat_rtmed, y=feat_mzmed, color=log10(feat_into)))
summarized_feats %>%
  ggplot() +
  geom_point(aes(x=mean_cor, y=mean_snr, color=log10(feat_into)))
summarized_feats %>%
  mutate(plot_col=feat_mzmed%between%pmppm(118.0865, 10)) %>%
  ggplot() +
  geom_point(aes(x=mean_cor, y=mean_snr, color=plot_col))
```

```{r}
library(squallms)
ms_peak_data <- msnexp_filled %>%
  makeXcmsObjFlat(revert_rts = FALSE) %>%
  filter(str_detect(filename, "Poo")) %>%
  mutate(filepath=str_replace(filepath, "G:\\\\My Drive\\\\", "C:\\\\Users\\\\willi\\\\Documents\\\\"))
labeled_feats <- squallms::labelFeatsLasso(ms_peak_data)


good_feats <- msnexp_filled %>%
  makeXcmsObjFlat(revert_rts = FALSE) %>%
  summarise(med_cor=median(beta_cor, na.rm = TRUE), med_snr=median(beta_snr, na.rm = TRUE), .by=feature) %>%
  squallms::logModelFeatQuality(feature_labels = labeled_feats, likelihood_threshold = 0.5)
```
