...
raw_gof <- read_table("raw_data/paragon1.gof", skip = 4, col_names = gof_names,
...
gm <- readRDS("raw_data/ggmap_of_aloha.rds")
...
seaflowdata <- readxl::read_excel("raw_data/SeaFlow_dataset_v1.4.xlsx")
seaflowdata %>%
...
```{r}
samp_filedata <- "../mzMLs" %>%
  list.files(pattern = "Smp.*mzML", recursive = TRUE) %>% 
  data.frame(filepath=.) %>%
  mutate(filename=basename(filepath)) %>%
  mutate(shortname=basename(filepath)) %>%
  mutate(shortname=str_remove(shortname, "-wIS|-noIS-neg|-noIS-pos")) %>%
  mutate(shortname=str_remove(shortname, ".*_Smp_")) %>%
  mutate(IS_type=str_extract(filepath, "wIS|noIS")) %>%
  mutate(polarity=dirname(filepath)) %>%
...
```{r write out}
write_csv(samp_filedata, file = "samp_filedata.csv")
```
